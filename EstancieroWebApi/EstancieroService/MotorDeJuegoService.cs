    ...
    public class MotorDeJuegoService
    {
        ...
        public int AvanzarTurno(int numeroPartida)
        {
         ...
         public ApiResponse<string> MoverJugador(int numeroPartida, int dniJugador)
         {
             ...
             // Si el tablero no está inicializado
                ...
                case TipoCasillero.Provincia:
                    {
                        ...
                        // Comprobar si DniPropietario es null/cero (si es int)
                        // Si DniPropietario es un int, la comprobación debe ser:
                        if (!casillero.DniPropietario.HasValue || casillero.DniPropietario.Value == 0) // Asumo DniPropietario es int?
                                                                                                       // Si DniPropietario es un string (como parece indicar el antiguo isNullOrEmpty):
                                                                                                       // if (string.IsNullOrEmpty(casillero.DniPropietario))
                        {
                            ...
                            if (precioCompra.HasValue && jugador.Saldo >= Convert.ToDouble(precioCompra.Value))
                            {
                                ...
                                // Asignación del DNI (si DniPropietario es int?)
                        ...
                        // Comparación directa de INT (si ambos son int)
